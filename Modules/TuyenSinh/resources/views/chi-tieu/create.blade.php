...
@section('page_title', 'Thêm Chỉ Tiêu Tuyển Sinh')
@section('content')
<div class="card">
    <div class="card-header"><h3 class="card-title">Thêm Chỉ Tiêu mới</h3></div>
    <div class="card-body">
        @if($errors->any())
        ...
        @endif
        <form action="{{ route('admin.chi-tieu.store') }}" method="POST">
            @csrf
            <div class="form-group">
                ...
                <select name="dot_tuyen_sinh_id" class="form-control">
                    ...
                    @foreach($dots as $dot)
                        <option value="{{ $dot->id }}" {{ old('dot_tuyen_sinh_id') == $dot->id ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <select name="phuong_thuc_id" class="form-control">
                    ...
                    @foreach($phuongThucs as $pt)
                        <option value="{{ $pt->id }}" {{ old('phuong_thuc_id') == $pt->id ? 'selected' : '' }}>
            ...
            <div class="row">
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <input type="number" name="chi_tieu_giao" class="form-control" value="{{ old('chi_tieu_giao') }}" min="1">
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="form-group">
                        <label>Đã tuyển</label>
                        <input type="number" name="chi_tieu_da_tuyen" class="form-control" value="{{ old('chi_tieu_da_tuyen', 0) }}" min="0">
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="form-group">
                        <label>Điểm chuẩn</label>
                        <input type="number" name="diem_chuan" class="form-control" value="{{ old('diem_chuan') }}" step="0.25" min="0">
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="form-group">
                        <label>Điểm sàn nộp HS</label>
                        <input type="number" name="diem_san_nop_hs" class="form-control" value="{{ old('diem_san_nop_hs') }}" step="0.25" min="0">
                    </div>
                </div>
            </div>
